...
# COPY mirage/pom.xml /dspace/modules/xmlui-mirage2/
...
COPY local.cfg /dspace-base/config/dspace.cfg
...
COPY --from=ANT_BUILD --chown=dspace:dspace /dspace-base/webapps/* $CATALINA_HOME/webapps/ROOT/
COPY --from=ANT_BUILD --chown=dspace:dspace /dspace-base/ /dspace-base
